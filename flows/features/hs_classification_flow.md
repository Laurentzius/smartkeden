...
* **In Scope:**
  ...
  - Qdrant collection `hs_code_directory` with 3072-dim Gemini embeddings.
...
flowchart TD
  ...
  Combined --> Embed[Gemini Embedding 3072-dim RETRIEVAL_QUERY]
...
flowchart LR
  ...
  Text --> Embedder[Gemini Embedding RETRIEVAL_QUERY]
  Embedder -->|3072-dim vector| Qdrant[(Qdrant hs_code_directory)]
...
* **Qdrant Collection `hs_code_directory`:**
  ...
  - Vector: 3072-dim embedding of product_name_ru + explanatory notes.
...
|Outgoing|`embedding_query`|Gemini Embedding|`{text, task_type: RETRIEVAL_QUERY}`|Text ready|API quota exceeded|
...
## 8. Side Effects
* **API Quota:** Each classification consumes 1 Vision call (optional), 1 Embedding call, 1 LLM structured output call to Gemini.
* **Rate Limiting:** Gemini API has per-minute quotas (typically 300 RPM for embedding, 30 RPM for generation). Batch large classification jobs with exponential backoff. If quota exceeded, queue and retry.
...
|:---|:---|:---|:---|
|Core / Unit|Embedding dimension validation (3072)|`backend/tests/test_vertex_client.py`|**PASSED**|
...
1. Create Qdrant collection `hs_code_directory` with same 3072-dim Cosine config.
...
* `backend/tests/test_hs_classifier.py` — 3 tests
...
### Status
* All 3 tests in `backend/tests/test_hs_classifier.py` pass
* Full suite: 34 tests pass
* Validation: `PYTHONPATH=backend .venv/Scripts/pytest backend/tests/ -q` → 34 passed in 3.15s
* Real embeddings blocked (needs GCP billing). Mock 3072-dim zeros used when embedding API unavailable.
---
