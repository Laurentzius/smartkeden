...
class NBKExchangeRatesService:
    ...
    def fetch_rates(cls) -> Dict[str, float]:
        ...
        try:
            logger.info(
                "Fetching daily exchange rates from National Bank of Kazakhstan"
            )
            ...
            for item in root.findall(".//item"):
                ...
                if title is not None and description is not None:
                    ...
                    try:
                        ...
                        quantity = (
                            float(quant.text.strip())
                            if quant is not None and quant.text
                            else 1.0
                        )
    ...
    def get_rate(cls, currency: str) -> float:
        ...
        if code not in rates:
            raise ValueError(
                f"Currency code '{currency}' not supported or found in NBK rates"
            )
        return rates[code]
