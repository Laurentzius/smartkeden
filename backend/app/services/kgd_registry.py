...
class KGDRegistryService:
    ...
    @staticmethod
    def search_brokers(db: Session, city: Optional[str] = None, query: Optional[str] = None) -> List[BrokerRegistry]:
        """
    ...
    def check_trois_trademark(db: Session, query_name: str) -> Optional[TROISRegistry]:
        ...
        trademark = db.query(TROISRegistry).filter(
            TROISRegistry.trademark_name.ilike(f"%{query_name}%")
        ).first()
        return trademark
