...
class CalculationRequest(BaseModel):
    invoice_price: float = Field(
        ..., description="Invoice price in foreign currency or KZT"
    )
    currency: str = Field(
        "USD", description="Three-letter currency code (USD, EUR, RUB, etc.)"
    )
    exchange_rate: Optional[float] = Field(
        None, description="Optional exchange rate; auto-resolved if omitted"
    )
    transport_to_border: float = Field(
        0.0, description="Transport cost to the EAEU/RK border in KZT"
    )
    duty_rate_percent: float = Field(
        0.0, description="Ad-valorem duty rate in percent (e.g., 10.0 for 10%)"
    )
    excise_rate_percent: float = Field(
        0.0, description="Ad-valorem excise rate in percent"
    )
    excise_specific_rate: float = Field(
        0.0, description="Specific excise rate (KZT per unit)"
    )
    excise_units_count: float = Field(
        0.0, description="Quantity of units subject to specific excise"
    )
    is_subject_to_recycling_fee: bool = Field(
        False, description="Whether the product is subject to recycling fee (утильсбор)"
    )
    recycling_fee_base_mci: float = Field(
        0.0, description="Base MCI (МРП) multiplier for recycling fee"
    )
    mci_value: Optional[float] = Field(
        None, description="Optional MCI value in KZT; auto-resolved if omitted"
    )
    declaration_date: Optional[str] = Field(
        None, description="Optional declaration date (YYYY-MM-DD) for historical rates"
    )


class CalculationResponse(BaseModel):
    customs_value_kzt: float = Field(
        ..., description="Customs value in KZT (Таможенная стоимость)"
    )
    customs_fee_kzt: float = Field(
        ..., description="Customs clearance fee (Таможенный сбор)"
    )
    customs_duty_kzt: float = Field(
        ..., description="Import/export customs duty (Таможенная пошлина)"
    )
    ...
    import_vat_kzt: float = Field(
        ..., description="Import VAT amount (16% of VAT Base)"
    )
    recycling_fee_kzt: float = Field(
        ..., description="Recycling fee amount (Утильсбор)"
    )
    total_payments_kzt: float = Field(
        ..., description="Total payments to be paid (Итого к уплате)"
    )
...
class CustomsCalculator:
    ...
    def calculate(cls, req: CalculationRequest) -> CalculationResponse:
        ...
        customs_fee_kzt = config_service.get_rate("customs_processing_fee")
        ...
        vat_base_kzt = (
            customs_value_kzt + customs_fee_kzt + customs_duty_kzt + excise_tax_kzt
        )
        try:
            import_vat_kzt = vat_base_kzt * config_service.get_rate(
                "import_vat", declaration_date=req.declaration_date
            )
        except Exception:
            import_vat_kzt = vat_base_kzt * rules.import_vat_rate
