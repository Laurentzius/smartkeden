            ...
            if boundary_re.match(line):
                ...
                if prefix.endswith('.') or re.match(r'^\d+\.', prefix):
                    ...
                    if len(subparts) > 1:
                        ...
                        words = line.split()
                        current_section = " ".join(words[:2]) if len(words) > 2 else line
                        current_content.append(line)
            else:
                current_content.append(line)
...
class DocumentParserRegistry:
    ...
    def get_parser(cls, doc_type: str) -> BaseDocumentParser:
        ...
        if parser is None:
            logger.warning("Unknown document type '%s'. Defaulting to CodeDocumentParser.", doc_type)
            return cls._parsers.get("code") or CodeDocumentParser()
        return parser

    @classmethod
    def parse(cls, raw_text: str, doc_title: str, doc_type: str = "code") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Error parsing document '%s' with parser '%s': %s", doc_title, doc_type, e)
            return []
