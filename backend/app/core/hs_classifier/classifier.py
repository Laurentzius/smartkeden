...
class HSCodeCandidate(BaseModel):
    ...
    is_subject_to_recycling_fee: bool = Field(
        False, description="Is subject to recycling fee (утильсбор)"
    )
...
class HSClassificationResponse(BaseModel):
    ...
    candidates: List[HSCodeCandidate] = Field(
        ..., description="Top HS code candidates sorted by confidence"
    )
    qdrant_backed: bool = Field(
        True, description="Whether candidates were grounded by Qdrant vector search"
    )


HS_CODE_COLLECTION = "hs_code_directory"


class HSCodeClassifier:
    """Multimodal HS Code Classifier.

    Implements: Multimodal LLM (Vision) -> Vector Search (Qdrant) -> LLM Selection.
    Dependencies are injected through the constructor — callers decide which
    embedding model and vector store to use.
    """

    def __init__(
        self, embedding_model: EmbeddingModel, vector_storage: VectorStorage
    ) -> None:
    ...
    async def classify(
        ...
        if image_bytes:
            ...
            try:
                ...
                vision_res = get_generator().generate_structured(
        ...
        # 2. Vector search candidate lookup via the VectorStorage seam
        logger.info(
            f"Performing vector search for description: {refined_description[:100]}..."
        )
        embedding = self._embedding_model.embed_text(
        ...
        try:
            search_result = self._vector_storage.query_points(
                collection_name=HS_CODE_COLLECTION,
            ...
            if search_result:
                    ...
                    qdrant_candidates.append(
                        {
                            ...
                            "excise_rate_percent": payload.get(
                                "excise_rate_percent", 0.0
                            ),
                            "is_subject_to_recycling_fee": payload.get(
                ...
                logger.info(
                    f"Found {len(qdrant_candidates)} candidates from vector store"
                )
        except Exception as e:
            logger.warning(f"Vector search failed, falling back to pure LLM: {e}")
